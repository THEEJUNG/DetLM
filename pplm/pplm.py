...
def run_pplm_example(
    ...
    if uncond:
        ...
        # open txt and make list
        cond_text_list = []
        with open(cond_text) as f:
            for i, line in enumerate(f):
                cond_text_list.append(line.replace("\n",""))
        # loop over prompt
        out_text = cond_text.replace(".txt","_out_%d.txt" %(class_label))
        w = open(out_text, 'a')
        for prompt in cond_text_list:
            ...
            w.write(unpert_gen_text.replace("\n",""))
            ...
            for i, pert_gen_tok_text in enumerate(pert_gen_tok_texts):
                try:
                    ...
                    w.write("\t"+pert_gen_text.replace("\n",""))
                    w.write("\t%d\n" %(class_label))
                except Exception as exc:
                    print("Ignoring error while generating perturbed text:", exc)

                # keep the prefix, perturbed seq, original seq for each index
                generated_texts.append((tokenized_cond_text,
                                        pert_gen_tok_text,
                                        unpert_gen_tok_text))
        w.close()
    return
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cond_text", type=str, help="Prefix text files")
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=("SentiTreeBank","HateOffensive"),
    ...
    DISCRIMINATOR_MODELS_PARAMS = {
        "HateOffensive":{
            ...
            "class_size": 3,
            "embed_size": 1024,
            "class_vocab": {'hate-speech':0, 'offensive-language':1, 'neither':2},
            "default_class": 2,
            "pretrained_model": "gpt2-medium"
        },
        "SentiTreeBank":{
            ...
            "class_size": 3,
            "embed_size": 1024,
            "class_vocab": {'negative':0, 'neutral':1, 'positive':2},
            "default_class": 1,
            "pretrained_model": "gpt2-medium"
        }
